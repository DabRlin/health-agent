...
const loadRagStats = async () => {
  try {
    const res = await api.medicalRagStats()
    if (res.success) ragStats.value = res.data
  } catch (e) { console.error('RAG stats failed:', e) }
}
...
const ragSearch = async () => {
  ...
  try {
    const res = await api.medicalRagSearch(ragSearchQuery.value.trim(), 5)
    if (res.success) {
...
const loadKnowledge = async () => {
  ...
  try {
    const res = await api.medicalListKnowledge(knowledgeFilter.value || undefined)
